#CECI EST UNE VERSION DE BACKUP POUR NOTRE MODULE DANS LE CAS OU NOUS DEVRIONS UTILISER L'IG BACKUP SUR NOTEBOOK.
#Merci de ne pas forcément en tenir compte pour la notation, la version au propre est module.py.
...
if __name__ == '__main__':
    ...
    for i in dict_besthit_proteine:
        ...
        if dict_besthit_proteine[i] != "Pas de résultat":
            list_proteine_info = recup_info_prot(i,dict_besthit_proteine[i].hit_id.translate(str.maketrans('', '', chars)))
    ...
    for i in dict_besthit_proteine_amont:
        ...
        if dict_besthit_proteine_amont[i] != "Pas de résultat":
            list_proteine_amont_info = recup_info_prot(i,dict_besthit_proteine_amont[i].hit_id.translate(str.maketrans('', '', chars)))
    ...
    for i in dict_besthit_proteine_aval:
        ...
        if dict_besthit_proteine_aval[i] != "Pas de résultat":
            list_proteine_aval_info = recup_info_prot(i,dict_besthit_proteine_aval[i].hit_id.translate(str.maketrans('', '', chars)))
    ...
    print(f"\nLes résultats de la synténie dans les 30 génomes sont désormais visible ! Pour cela reportez vous à la partie visualisation, une image est aussi accessible dans le dossier où vous avez lancé l'interface.")
